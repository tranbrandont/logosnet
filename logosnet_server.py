    ...
    while 1:
        ...
        for sock in read:
            if sock == serv_sock:
                ...
                if username:
                    _messagesize, username = struct.unpack('!I%ds' % ((psize - 4),), username)
                ...
                try:
                    ...
                    if message:
                        ...
                        print(message)
                        broadcast(serv_sock, sock, "\r" + '[' + str(sock.getpeername()) + '] ' + message)
                    else:
                    ...
                    SOCK_LIST.remove(sock)
                    broadcast(serv_sock, sock, "Client (%s, %s) is offline, try failed\n" % _addr)
                    sock.close()
    serv_sock.close()
